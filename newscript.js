...
const allCalculatorButtonsArr = document.querySelectorAll(
  ".calculator__button"
);

// adding numberbuttons class to register numper input
const NumberButtonInputArr = document.querySelectorAll(
  ".calculator__button--numeric"
);

// adding operand buttons input
const operandButtonInputtArr = document.querySelectorAll(
...
const calculator = {
  ...
  handleEqualsButton(handleNewOperand) {
    this.currentNumberNum += parseFloat(this.currentNumberString);
    (this.calcAswer = this.runningTotal), this.handleNewOperand();
    this.runningTotal = 0;
    this.currentNumberString = 0;
    this.currentNumberNum = 0;
    this.currentOperand = "";
...
for (let index = 0; index < allCalculatorButtonsArr.length; index++) {
  allCalculatorButtonsArr[index].addEventListener(
    "click",
    calculator.changeOutputScreenDisplay
  );
}
for (let index = 0; index < NumberButtonInputArr.length; index++) {
  NumberButtonInputArr[index].addEventListener(
    "click",
    calculator.handleNumberInput
  );
}
for (let index = 0; index < operandButtonInputtArr.length; index++) {
  operandButtonInputtArr[index].addEventListener(
    "click",
    calculator.handleOperandInput(calculator.handleNewOperand)
  );
}
equalsButtonInput.addEventListener(
  "click",
  calculator.handleEqualsButton(calculator.handleNewOperand)
);
